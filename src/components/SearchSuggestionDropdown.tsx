...
export const SearchSuggestionDropdown: React.FC<SearchSuggestionDropdownProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* 2. REAL MUSIC TRACK PREDICTIONS (MUSIC-FIRST) */}
      {hasSongs && (
        <div className="p-3 space-y-1">
          <div className="px-2 py-1 flex items-center justify-between">
            <span className="text-[11px] font-bold uppercase tracking-wider text-[#888888] flex items-center gap-1.5">
              <Music className="w-3 h-3 text-[#ff0000]" />
              <span>Real Music Tracks</span>
            </span>
            <span className="text-[10px] text-[#22c55e] font-semibold bg-[#22c55e]/10 px-1.5 py-0.5 rounded-full">
              Studio Master
            </span>
          </div>
      ...
      {hasSuggestions && (
        <div className="p-3 space-y-1">
          ...
          <div className="space-y-0.5">
            {data.suggestions.slice(0, 5).map((q) => (
              <div
                key={q}
                onClick={() => onSelectQuery(q)}
