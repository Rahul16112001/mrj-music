...
import { searchYouTubeInnertube } from './youtubeInnertubeClient.js';
...
export const trackIdentityManager = {
  ...
  async fetchAndResolveSource(canonicalTrack, targetFormat = 'audio') {
    ...
    try {
      const primaryQuery = targetFormat === 'audio' 
        ? `${canonicalTrack.title} ${canonicalTrack.artist} official audio`
        : `${canonicalTrack.title} ${canonicalTrack.artist} official video`;
      const fallbackQuery = `${canonicalTrack.title} ${canonicalTrack.artist}`;

      const [primaryRes, fallbackRes] = await Promise.allSettled([
        searchYouTubeInnertube(primaryQuery, 15),
        searchYouTubeInnertube(fallbackQuery, 15),
      ]);

      const allCandidates = [];
      const seenIds = new Set();

      const addCandidates = (res) => {
        if (res.status === 'fulfilled' && Array.isArray(res.value?.rawCandidates)) {
          for (const cand of res.value.rawCandidates) {
      ...
      addCandidates(primaryRes);
      addCandidates(fallbackRes);
