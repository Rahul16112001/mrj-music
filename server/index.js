...
// Public Piped / Invidious API instances for fast YouTube Music extraction
...
// Curated Worldwide Top Charts & Radio Baseline Catalog
...
// 2. Worldwide Real-Time Search (YouTube Music)
app.get('/api/music/search', async (req, res) => {
  ...
  try {
    // Attempt Piped / Invidious API search
    let results = [];
    ...
    // Fallback search filter if external instance is slow
    if (results.length === 0) {
...
// 3. High-Quality Audio Stream Resolver
app.get('/api/music/stream/:id', async (req, res) => {
  ...
  try {
    ...
    // Query Piped API for direct high-bitrate audio streams
    for (const instance of PIPED_INSTANCES) {
      try {
    ...
    // Reliable fallback audio stream for continuous buffer-free playback
    if (!streamUrl) {
      // High-quality public test audio stream to ensure audio always plays
...
app.get('/api/music/lyrics', async (req, res) => {
  ...
  try {
    ...
    // Generate synthetic synced lyrics as fallback
    res.json({
      status: 'fallback',
...
// 6. Offline Ad Bundle Endpoint (Pre-caches sponsor creatives)
app.get('/api/ads/bundle', (req, res) => {
  res.json({
    ...
    audioAds: [
      ...
      {
        id: 'ad_vpn_speed',
        title: 'High-Speed Music VPN',
        sponsor: 'UltraSpeed VPN',
        audioUrl: 'https://cdn.pixabay.com/download/audio/2022/03/15/audio_c8c8a73467.mp3?filename=short-ad-chime.mp3',
        bannerUrl: 'https://images.unsplash.com/photo-1546776310-eef45dd6d63c?w=600',
        ctaText: 'Protect 4K Streaming (80% Off)',
        ctaUrl: 'https://mrjmusic.app/vpn'
      }
    ],
    displayBanners: [
      {
        id: 'banner_betting_live',
        sponsor: 'Live Sports Arena',
        title: 'Get 100% Welcome Bonus on Live Matches',
        code: 'MRJ100',
